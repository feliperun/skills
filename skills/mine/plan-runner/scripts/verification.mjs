...
import { closeSync, lstatSync, mkdirSync, openSync, readlinkSync, readSync, readdirSync, realpathSync, statSync, writeFileSync } from "node:fs";
 ...
 *
 * @typedef {{path: string, kind: "directory"|"file"|"symlink", digest?: string, size?: number}} SnapshotEntry
 */

/** @typedef {{entries: SnapshotEntry[], truncated: boolean}} WorkspaceSnapshot */
 ...
 */
export function compareWorkspaceSnapshot(before, cwd, allowedPaths = []) {
  if (!before || !Array.isArray(before.entries) || before.truncated) {
  ...
  const allowed = new Set(allowedPaths.map((path) => String(path).replaceAll("\\", "/")));
