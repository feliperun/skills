...
export function judgePrompt(node, workerResult, context = {}) {
  ...
  return `Review node ${node.id} independently. The review context is closed: inspect only the ${node.taskPacket.mode === "autonomous" ? "write roots" : "write files"} below, run only the verification commands below, and do not perform repository-wide discovery.\n\n` +
    `${writeBoundaryLabel}:\n${writeFiles}\n\nVerification:\n${verification}\n\n` +
