...
export async function driveRun(contract, runDir, states, campaign, lease, sourceIdentity) {
  ...
  try {
    while ([...states.values()].some((state) => !TERMINAL.has(state.status))) {
      ...
      enforceTokenBudget(contract, runDir, states, lease, campaign.path);
...
async function finalizeClosedJobs(contract, runDir, states, running, lease, campaignPath) {
  for (const [nodeId, job] of running) {
    ...
    // Read the provider close evidence before the scope gate so every terminal
    // settlement from this close path — including a scope failure — persists
    // the provider receipts and usage.
 ...
 * @param {string} campaignPath
 */
function enforceTokenBudget(contract, runDir, states, lease, campaignPath) {
...
function reusedDoneWarnings(contract) {
  ...
  for (const name of readdirSync(runsDir)) {
    ...
    let otherContract;
    try {
      otherContract = validateContract(JSON.parse(readFileSync(otherContractPath, "utf8")), otherContractPath);
    } catch {
      continue;
    }
