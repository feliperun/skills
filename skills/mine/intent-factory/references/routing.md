# Routing and operations reference

Everything the [SKILL.md](../SKILL.md) router summarizes: the continuity
kernel, the full workflow, the runtime catalogue, gate tuning, the dashboard,
and the v0 limitations. The router carries the rules; this file carries the
detail behind them.
...
are documented in [budget-governance.md](budget-governance.md).
...
2. Establish the durable campaign before inspecting or launching work. Discover
   ...
   [session-memory.md](session-memory.md). The
...
(`provider_exhausted`, `provider_exhausted_without_declared_failover`,
`quota_exhausted`, `payment_required` — all `requiresUser: true`); it never
changes provider or account implicitly. Failover is used only for
