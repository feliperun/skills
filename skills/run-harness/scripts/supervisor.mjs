...
import { spawn } from "node:child_process";
const config = JSON.parse(readFileSync(process.env.HARNESS_GATE_CONFIG, "utf8"));
const releasePath = process.env.HARNESS_GATE_RELEASE;
const parentPid = Number(process.env.HARNESS_GATE_PARENT_PID);
const parentToken = process.env.HARNESS_GATE_PARENT_TOKEN || null;
...
export function startProcess({ contract, node, state, runtime, prompt, paths, phase, commandOptions = {}, onInvocation }) {
  ...
  try {
    child = spawn(process.execPath, ["-e", GATE_SCRIPT], {
      ...
      env: {
        ...process.env,
        HARNESS_GATE_CONFIG: gateConfigPath,
        HARNESS_GATE_RELEASE: gateReleasePath,
        HARNESS_GATE_PARENT_PID: String(process.pid),
        HARNESS_GATE_PARENT_TOKEN: processStartToken(process.pid) ?? "",
      },
