...
import { HARNESS_VERSION, PROTOCOL_SCHEMA_VERSION, probeRuntime } from "./drivers/index.mjs";
...
export async function driveRun(contract, runDir, states, campaign, lease, sourceIdentity) {
  ...
  const detachedBootstrap = validBootstrapNonce(process.env.HARNESS_BOOTSTRAP_NONCE);
...
async function acquireStaleControllerLease(runDir) {
  for (;;) {
    try {
      return acquireControllerLease(runDir, { harnessVersion: HARNESS_VERSION, processStartToken: processStartToken(process.pid) });
    } catch (error) {
...
function detachSelf(command, target, extraArgs = []) {
  ...
  const child = /** @type {DetachedChild} */ (spawn(process.execPath, [fileURLToPath(import.meta.url), command, target, ...extraArgs], {
    cwd: process.cwd(),
    env: { ...process.env, HARNESS_BOOTSTRAP_NONCE: nonce },
...
function writeBootstrapFailure(command, target, error) {
  ...
  const nonce = validBootstrapNonce(process.env.HARNESS_BOOTSTRAP_NONCE) ? process.env.HARNESS_BOOTSTRAP_NONCE : null;
...
function bootstrapNonceForProcess() {
  return validBootstrapNonce(process.env.HARNESS_BOOTSTRAP_NONCE)
    ? /** @type {string} */ (process.env.HARNESS_BOOTSTRAP_NONCE)
    : randomUUID();
}
...
const DRIVER_BIN_OVERRIDES = Object.freeze({
  codex: "HARNESS_CODEX_BIN",
  claude: "HARNESS_CLAUDE_BIN",
  agy: "HARNESS_AGY_BIN",
  "exec-jsonl": "HARNESS_EXEC_JSONL_BIN",
});
...
async function doctorCommand(contractPath, values) {
  ...
  checks.push({ name: "harness schema", ok: true, detail: `protocol ${PROTOCOL_SCHEMA_VERSION} · harness ${HARNESS_VERSION}` });
  /** @type {Set<string>} */
  ...
  // an explicit executable or a HARNESS_*_BIN override; the driver probe above
...
function usage() {
  process.stderr.write(
    "usage: harness.mjs <run|validate|preflight> <contract.json> [--detach] | " +
