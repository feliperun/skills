...
/** @typedef {{schemaVersion: number, harnessVersion: string, holderId: string, generation: number, pid: number, processStartToken: string|null, acquiredAt: string, renewedAt: string, expiresAt: string, invalid?: never}} LeaseRecord */
/** @typedef {LeaseRecord|null|{invalid: true, path: string}} ReadLeaseResult */
/** @typedef {{holderId?: string, pid?: number, ttlMs?: number, harnessVersion?: string, processStartToken?: string|null, now?: number, fileName?: string, onRenew?: (lease: LeaseRecord) => void}} LeaseOptions */
...
export function leaseHealthy(lease, now = Date.now()) {
  return Boolean(
    ...
    typeof /** @type {Record<string, unknown>} */ (lease).harnessVersion === "string" &&
 ...
 * @returns {ReturnType<typeof createLeaseHandle>}
 */
function acquireLease(runDir, options) {
  ...
  const harnessVersion = options.harnessVersion ?? "unknown";
  let generation = 1;
  try {
