...
import { HARNESS_VERSION, PROTOCOL_SCHEMA_VERSION, validateContract, validateNodeSnapshot } from "../scripts/contract.mjs";
...
test("simultaneous controllers have one exclusive lease and stale takeover increments generation", () => {
  const runDir = mkdtempSync(join(tmpdir(), "harness-lease-"));
  const first = acquireControllerLease(runDir, { harnessVersion: "0.1.0", ttlMs: 50, now: 1_000 });
  assert.throws(
    () => acquireControllerLease(runDir, { harnessVersion: "0.1.0", ttlMs: 50, now: 1_010 }),
    (error) => error instanceof LeaseBusyError,
  );
  const second = acquireControllerLease(runDir, { harnessVersion: "0.1.0", ttlMs: 50, now: 1_100 });
...
test("a delayed heartbeat renews its own expired lease instead of declaring it lost", async () => {
  const runDir = mkdtempSync(join(tmpdir(), "harness-heartbeat-delay-"));
  const lease = acquireControllerLease(runDir, { harnessVersion: "0.1.0", ttlMs: 100 });
  let lost = false;
...
test("lease heartbeat expiry is authoritative even when the old PID is still alive", () => {
  const runDir = mkdtempSync(join(tmpdir(), "harness-heartbeat-"));
  const first = acquireControllerLease(runDir, { harnessVersion: "0.1.0", ttlMs: 100, now: 1_000 });
  writeJsonAtomic(join(runDir, "controller-lease.json"), {
    ...first.current,
    renewedAt: new Date(500).toISOString(),
    expiresAt: new Date(900).toISOString(),
  });
  const second = acquireControllerLease(runDir, { harnessVersion: "0.1.0", ttlMs: 100, now: 1_000 });
  assert.equal(second.generation, first.generation + 1);
...
test("watcher lease takeover reclaims stale ownership without a permanent lock", () => {
  const runDir = mkdtempSync(join(tmpdir(), "harness-watcher-lease-"));
  const first = acquireWatcherLease(runDir, { harnessVersion: "0.1.0", ttlMs: 50, now: 1_000 });
  const second = acquireWatcherLease(runDir, { harnessVersion: "0.1.0", ttlMs: 50, now: 1_100 });
  assert.equal(second.generation, first.generation + 1);
  first.release();
...
test("atomic JSON and JSONL recovery never leaves a partial authoritative record", () => {
  const runDir = mkdtempSync(join(tmpdir(), "harness-store-"));
...
test("JSONL append recovers a truncated final record before appending", () => {
  const runDir = mkdtempSync(join(tmpdir(), "harness-jsonl-recovery-"));
  const jsonlPath = join(runDir, "events.jsonl");
...
test("stall supervision uses the latest persisted timeout override", async () => {
  const runDir = mkdtempSync(join(tmpdir(), "harness-timeout-override-"));
  const logs = join(runDir, "logs");
  mkdirSync(logs);
  const marker = join(runDir, "provider-started");
  const provider = join(runDir, "provider.mjs");
  writeFileSync(provider, "import { writeFileSync } from \"node:fs\"; writeFileSync(process.env.HARNESS_MARKER, \"started\"); process.stdin.resume(); setTimeout(() => {}, 1000);\n");
  chmodSync(provider, 0o755);
  const previous = process.env.HARNESS_CODEX_BIN;
  const previousMarker = process.env.HARNESS_MARKER;
  process.env.HARNESS_CODEX_BIN = provider;
  process.env.HARNESS_MARKER = marker;
  ...
  let timeout;
    ...
    if (previousMarker === undefined) delete process.env.HARNESS_MARKER;
    else process.env.HARNESS_MARKER = previousMarker;
...
test("a persistence failure leaves the gated provider unstarted and terminates its wrapper", async () => {
  const runDir = mkdtempSync(join(tmpdir(), "harness-persistence-barrier-"));
  const logs = join(runDir, "logs");
  mkdirSync(logs);
  const marker = join(runDir, "provider-started");
  const provider = join(runDir, "provider.mjs");
  writeFileSync(provider, "import { writeFileSync } from \"node:fs\"; writeFileSync(process.env.HARNESS_MARKER, \"started\"); setInterval(() => {}, 1000);\n");
  chmodSync(provider, 0o755);
  const previous = process.env.HARNESS_CODEX_BIN;
  const previousMarker = process.env.HARNESS_MARKER;
  process.env.HARNESS_CODEX_BIN = provider;
