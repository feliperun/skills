...
import { runContract } from "../scripts/harness.mjs";
...
test("semantic budget keeps critical sections and evicts oldest low-priority history above 16 KiB", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-budget-"));
...
test("many linked runs cannot starve critical handoff sections", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-run-flood-"));
  const runsDir = join(directory, ".runs");
...
test("attention-needed run states survive budget pressure with an omission note", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-attention-flood-"));
  const runsDir = join(directory, ".runs");
...
test("active decisions and unresolved questions beyond twenty are preserved when they fit", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-beyond-cap-"));
  const runsDir = join(directory, ".runs");
...
test("decisions evicted by the projection cap still produce an omission summary", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-evicted-decisions-"));
  const runsDir = join(directory, ".runs");
...
test("a critical section larger than the whole budget keeps its latest entries", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-critical-"));
  const runsDir = join(directory, ".runs");
...
test("fitHandoff shrinks entry text until every critical entry survives", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-shrink-"));
  const runsDir = join(directory, ".runs");
...
test("oldest low-priority history is evicted first with a bounded omission summary", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-evict-"));
  const runsDir = join(directory, ".runs");
...
test("resolved questions leave the active handoff projection", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-question-"));
  const runsDir = join(directory, ".runs");
...
test("campaigns close and implicit discovery considers only active campaigns", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-close-"));
  const runsDir = join(directory, ".runs");
...
test("corrupt campaign entries are surfaced instead of silently dropped", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-corrupt-"));
  const runsDir = join(directory, ".runs");
...
test("journal appends are idempotent by event id", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-dedupe-"));
  const runsDir = join(directory, ".runs");
...
test("handoff projection recovers from deletion and corruption", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-projection-"));
  const runsDir = join(directory, ".runs");
...
test("journal text is normalized and bounded so entries cannot inject headings", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-normalize-"));
  const runsDir = join(directory, ".runs");
...
test("attention-needed linked-run states survive when critical sections exhaust the budget", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-attention-critical-"));
  const runsDir = join(directory, ".runs");
...
test("large valid identifiers cannot starve later critical sections", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-large-ids-"));
  const runsDir = join(directory, ".runs");
...
test("projection recovery after a partial trailing journal line beyond the tail window does not duplicate", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-projection-tail-"));
  const runsDir = join(directory, ".runs");
...
test("a stale projection record with byte 0 but folded entries is reparsed, not duplicated", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-projection-stale-"));
  const runsDir = join(directory, ".runs");
...
test("session lineage beyond 20 sessions survives when the budget allows", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-sessions-bounded-"));
  const runsDir = join(directory, ".runs");
...
test("campaign CLI lists, closes, and resolves questions", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-cli-lifecycle-"));
  const harness = fileURLToPath(new URL("../scripts/harness.mjs", import.meta.url));
  const initA = spawnSync(process.execPath, [harness, "campaign", "init", "alpha", "--cwd", directory, "--goal", "Ship A"], { encoding: "utf8" });
  assert.equal(initA.status, 0, initA.stderr);
  const initB = spawnSync(process.execPath, [harness, "campaign", "init", "beta", "--cwd", directory, "--goal", "Ship B"], { encoding: "utf8" });
  assert.equal(initB.status, 0, initB.stderr);

  const listed = spawnSync(process.execPath, [harness, "campaign", "list", "--cwd", directory], { encoding: "utf8" });
  ...
  const note = spawnSync(process.execPath, [
    harness, "campaign", "note", "alpha", "--cwd", directory,
  ...
  const resolved = spawnSync(process.execPath, [
    harness, "campaign", "resolve", "alpha", "--cwd", directory,
    "--session-id", "codex-1", "--question-id", "q1", "--text", "Yes, semantically.",
  ], { encoding: "utf8" });
  assert.equal(resolved.status, 0, resolved.stderr);
  const show = spawnSync(process.execPath, [harness, "campaign", "show", "alpha", "--cwd", directory], { encoding: "utf8" });
  assert.equal(show.status, 0, show.stderr);
  assert.doesNotMatch(show.stdout, /Is the handoff bounded\?/u);

  const closed = spawnSync(process.execPath, [harness, "campaign", "close", "beta", "--cwd", directory], { encoding: "utf8" });
  assert.equal(closed.status, 0, closed.stderr);
  assert.match(closed.stdout, /beta closed/u);
  const listedAgain = spawnSync(process.execPath, [harness, "campaign", "list", "--cwd", directory], { encoding: "utf8" });
  ...
  const noteOnClosed = spawnSync(process.execPath, [
    harness, "campaign", "note", "beta", "--cwd", directory,
...
test("campaign CLI attach retries with a stable event id are idempotent", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-cli-idem-"));
  const harness = fileURLToPath(new URL("../scripts/harness.mjs", import.meta.url));
  const init = spawnSync(process.execPath, [harness, "campaign", "init", "idem", "--cwd", directory, "--goal", "Ship"], { encoding: "utf8" });
  assert.equal(init.status, 0, init.stderr);
  const args = [
    harness, "campaign", "attach", "idem", "--cwd", directory,
    "--tool", "codex", "--session-id", "codex-1", "--transcript", join(directory, "transcript.jsonl"),
...
test("campaign CLI accepts --no-transcript and rejects unknown options", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-cli-strict-"));
  const harness = fileURLToPath(new URL("../scripts/harness.mjs", import.meta.url));
  const init = spawnSync(process.execPath, [harness, "campaign", "init", "strict", "--cwd", directory, "--goal", "Ship"], { encoding: "utf8" });
  assert.equal(init.status, 0, init.stderr);
  const attach = spawnSync(process.execPath, [
    harness, "campaign", "attach", "strict", "--cwd", directory,
    "--tool", "codex", "--session-id", "codex-1", "--no-transcript",
  ], { encoding: "utf8" });
  assert.equal(attach.status, 0, attach.stderr);
  const unknown = spawnSync(process.execPath, [
    harness, "campaign", "note", "strict", "--cwd", directory,
...
test("campaign CLI scopes flags to operations and note kinds", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-cli-scoped-"));
  const harness = fileURLToPath(new URL("../scripts/harness.mjs", import.meta.url));
  const init = spawnSync(process.execPath, [harness, "campaign", "init", "scoped", "--cwd", directory, "--goal", "Ship"], { encoding: "utf8" });
  assert.equal(init.status, 0, init.stderr);
  const listWithGoal = spawnSync(process.execPath, [harness, "campaign", "list", "--cwd", directory, "--goal", "ignored"], { encoding: "utf8" });
  assert.notEqual(listWithGoal.status, 0);
  assert.match(listWithGoal.stderr, /Unknown option '--goal'/u);
  const showWithGoal = spawnSync(process.execPath, [harness, "campaign", "show", "scoped", "--cwd", directory, "--goal", "ignored"], { encoding: "utf8" });
  ...
  const noteWithWrongKindFlag = spawnSync(process.execPath, [
    harness, "campaign", "note", "scoped", "--cwd", directory,
...
test("linked-run corruption cannot kill a controller", async () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-corrupt-run-"));
  const path = writeContract(directory, fixture({
    id: "clean-run",
    pollIntervalMs: 10,
    nodes: [{ id: "build", type: "backend", taskPacket: packet(), gate: false }],
...
test("handoff projection failure records a diagnostic and cannot kill a controller", async () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-campaign-diagnostic-"));
  const path = writeContract(directory, fixture({
    id: "diag-run",
    pollIntervalMs: 10,
    nodes: [{ id: "build", type: "backend", taskPacket: packet(), gate: false }],
...
test("appendJsonl repairs a partial trailing line from a bounded tail", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-jsonl-repair-"));
...
test("appendJsonl recovers a large journal from its bounded tail", () => {
  const directory = mkdtempSync(join(tmpdir(), "harness-jsonl-repair-tail-"));
  const path = join(directory, "events.jsonl");
